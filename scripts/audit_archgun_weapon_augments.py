#!/usr/bin/env python3
"""Audit archgun weapon-exclusive augments: behaviors, stats, wiki sync, metadata."""
...
ARCHGUN_CATEGORIES = {"archgun"}

# Legacy miscategorizations that should not be category archgun.
ARCHGUN_CATEGORY_BLOCKLIST = {
    "argent_scourge",
}
...
def is_archgun_weapon_augment(
    ...
    if wids:
        ...
        cats.discard(None)
        if cats and cats <= ARCHGUN_CATEGORIES:
            return True
        if cats and not cats <= ARCHGUN_CATEGORIES:
            return False
    if m.get("subCategory") == "weapon" and m["category"] == "archgun":
        return True
    entry = wiki_entry(wiki, m)
    if entry and entry.get("wikiType", "").lower() in {"archgun", "arch-gun"}:
        return False  # universal archgun mods, not weapon-exclusive
    return False


def archgun_category_pollution(mods: list[dict], wiki: dict) -> list[str]:
    issues: list[str] = []
    for m in mods:
        if m["category"] != "archgun":
            continue
        if m["id"] in ARCHGUN_CATEGORY_BLOCKLIST:
            continue
        entry = wiki_entry(wiki, m)
        if not entry:
            continue
        wt = (entry.get("wikiType") or "").strip().lower()
        if wt and wt not in {"archgun", "arch-gun"}:
            issues.append(f"{m['id']}: category archgun but wiki Type={entry.get('wikiType')!r}")
    return issues
...
def main() -> None:
    ...
    archgun_ids = {w["id"] for w in weapons if w.get("category") == "archgun"}
    exclusive = load_exclusive()
    behaviors = load_behaviors()
    wiki = load_wiki()

    pool = [m for m in mods if is_archgun_weapon_augment(m, exclusive, wiki, weapons, weapon_cat)]

    print("=== Archgun weapon-exclusive augment audit ===\n")
    print(f"Archgun weapons in catalog: {len(archgun_ids)}")
    print(f"Weapon-exclusive augment pool: {len(pool)}")
    if pool:
    ...
    for m in mods:
        ...
        if wids and all(weapon_cat.get(w) == "archgun" for w in wids):
            wiki_archgun_augments += 1
    print(f"Wiki IsWeaponAugment entries for archgun weapons: {wiki_archgun_augments}")

    exclusive_touching = [
        (mod_id, wids)
        for mod_id, wids in exclusive.items()
        if any(w in archgun_ids for w in wids)
    ]
    print(f"MOD_EXCLUSIVE_WEAPON_IDS entries for archguns: {len(exclusive_touching)}")
    ...
    for m in pool:
        ...
        if m["id"] in exclusive and m.get("subCategory") != "weapon":
            metadata_issues.append(
                f"{m['id']}: exclusive augment should have subCategory weapon (has {m.get('subCategory')!r})"
            )
    ...
    pollution = archgun_category_pollution(mods, wiki)
    blocklist_in_archgun = [
        m for m in mods
        if m["category"] == "archgun" and m["id"] in ARCHGUN_CATEGORY_BLOCKLIST
    ]
    print(f"\nMiscategorized archgun-category mods (blocklist): {len(blocklist_in_archgun)}")
    for m in sorted(blocklist_in_archgun, key=lambda x: x["name"]):
        entry = wiki_entry(wiki, m)
        wt = entry.get("wikiType") if entry else "?"
        print(f"  {m['id']}: {m['name']} (wiki Type={wt!r})")

    print(f"\nArchgun pool pollution (wrong wiki Type): {len(pollution)}")
    for line in sorted(pollution):
        print(f"  {line}")
    ...
    issues = (
        ...
        + len(metadata_issues)
        + len(pollution)
        + len(blocklist_in_archgun)
        + len(exclusive_touching)
    )
