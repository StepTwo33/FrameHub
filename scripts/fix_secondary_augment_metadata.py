#!/usr/bin/env python3
"""Tag weapon-exclusive secondary mods with subCategory weapon for consistent filtering."""
...
def load_secondary_weapon_ids() -> set[str]:
...
def main() -> None:
    secondary_ids = load_secondary_weapon_ids()
    exclusive = load_exclusive()
    secondary_exclusive_ids = {
        mod_id
    ...
    updated: list[str] = []
    for mod in mods:
        if mod["id"] not in secondary_exclusive_ids:
            continue
    ...
    print(f"Tagged subCategory weapon on {len(updated)} secondary-exclusive mods")
