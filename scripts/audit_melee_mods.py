#!/usr/bin/env python3
"""Audit regular melee mods: behaviors, stats, wiki sync."""
...
def main() -> None:
    ...
    pool = [
        m for m in mods
        if m["category"] in MELEE_CATEGORIES
        and m["id"] not in exilus_ids
        and m["id"] not in TOME_IDS
        and m["subCategory"] != "riven"
        and m["subCategory"] != "weapon"
        and m["category"] != "stance"
        and not m["id"].startswith("historic_")
        and not m["id"].startswith("stance_")
    ]

    print("=== Regular melee mod audit ===\n")
    print(f"Pool size (exilus + tome + riven + weapon-augments + stance excluded): {len(pool)}")
    ...
    print(f"\nWeapon-exclusive mods in regular melee pool: {len(exclusive_in_pool)}")
