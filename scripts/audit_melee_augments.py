#!/usr/bin/env python3
"""Audit melee weapon augments: behaviors, stats, wiki sync, metadata."""
...
MELEE_CATEGORIES = {"melee", "beast_claw"}

# Stance mods mis-tagged as weapon-exclusive in generated tags.
STANCE_MOD_IDS = {
    "atlantis_vulcan",
    "mafic_rain",
}
...
def is_melee_weapon_augment(
    ...
    if wids:
        ...
        cats.discard(None)
        if cats and cats <= MELEE_CATEGORIES:
            return True
        if cats and not cats <= MELEE_CATEGORIES:
            return False
    if m["subCategory"] == "weapon" and m["category"] == "melee":
        return True
    entry = wiki_entry(wiki, m)
    if entry and entry.get("wikiType", "").lower() == "melee":
        return True
    return False
...
def main() -> None:
    ...
    pool = [m for m in mods if is_melee_weapon_augment(m, exclusive, wiki, weapons, weapon_cat)]

    print("=== Melee weapon augment audit ===\n")
    print(f"Pool size: {len(pool)}")
    print(f"By category: {dict(Counter(m['category'] for m in pool))}")
    print(f"subCategory weapon: {sum(1 for m in pool if m['subCategory'] == 'weapon')}")
    ...
    wrong_sub = [m for m in pool if m["subCategory"] != "weapon" and m["id"] not in exclusive]
    if wrong_sub:
        print(f"\nMelee augments missing subCategory weapon or exclusive map ({len(wrong_sub)}):")
        for m in sorted(wrong_sub, key=lambda x: x["name"]):
            print(f"  {m['id']}: category={m['category']} sub={m['subCategory']!r}")

    not_in_exclusive = [
        m for m in pool
        if m["id"] not in exclusive and m["subCategory"] == "weapon"
    ]
    if not_in_exclusive:
        print(f"\nWeapon-tagged melee augments not in MOD_EXCLUSIVE_WEAPON_IDS ({len(not_in_exclusive)}):")
    ...
    for m in pool:
        ...
        if m["id"] in exclusive and m["subCategory"] != "weapon" and m["category"] != "melee":
            metadata_issues.append(
                f"{m['id']}: exclusive melee augment should have subCategory weapon (has {m['subCategory']!r})"
            )
