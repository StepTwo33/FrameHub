#!/usr/bin/env python3
"""Audit companion precept (penjaga) mods — companion 'augments'/abilities."""
...
def main() -> None:
    ...
    # Precepts: penjaga polarity companion mods (ability slots)
    pool = [
        m for m in mods
        if m["category"] == "companion"
        and m["polarity"] == "penjaga"
        and m["subCategory"] != "riven"
        and not m["id"].startswith("historic_")
    ]

    print("=== Companion precept (penjaga) audit ===\n")
